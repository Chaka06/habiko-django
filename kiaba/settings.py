...
if "test" in sys.argv or env("DB_ENGINE") == "sqlite" or "pytest" in sys.modules:
    ...
    if database_url:
        ...
        _disable_pooler = os.environ.get("DISABLE_POOLER_CONVERSION", "").lower() in ("1", "true", "yes")
        if _vercel and not _disable_pooler and "db." in database_url and ".supabase.co" in database_url and "pooler.supabase.com" not in database_url:
            import re
...
# DRF (throttling for rate limiting contact clicks)
REST_FRAMEWORK = {
...
SESSION_SAVE_EVERY_REQUEST = False  # ConsumeMessagesAfterResponseMiddleware gère la suppression des messages
...
GENIUSPAY_API_KEY        = os.environ.get("GENIUSPAY_API_KEY", "").strip()
GENIUSPAY_API_SECRET     = os.environ.get("GENIUSPAY_API_SECRET", "").strip()
# Secret webhook (whsec_sandbox_xxx ou whsec_live_xxx)
GENIUSPAY_WEBHOOK_SECRET = os.environ.get("GENIUSPAY_WEBHOOK_SECRET", "").strip()

# ── PawaPay (conservé pour compatibilité — remplacé par GeniusPay) ────────────
