...
def _ads_posted_last_hour(user) -> int:
    """Nombre d'annonces créées par l'utilisateur dans la dernière heure."""
    one_hour_ago = timezone.now() - timezone.timedelta(hours=1)
    return Ad.objects.filter(user=user, created_at__gte=one_hour_ago).count()
...
def post(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        # Rate limit : max 3 annonces par heure
        if _ads_posted_last_hour(request.user) >= 3:
            messages.error(
                request,
                "Limite atteinte : vous ne pouvez pas publier plus de 3 annonces par heure. "
        ...
        if form.is_valid():
            ...
            # Créer l'annonce en DRAFT — elle sera activée après paiement
            ...
            # Traitement des images (filigrane + miniature) — asynchrone si Redis disponible
            ...
            # Stocker l'id de l'annonce en session pour le paiement
